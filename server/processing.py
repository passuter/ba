"""
Author: Pascal Suter
This module processes data generated by a test.
"""
...
def begin_processing(): 
    ...
    process(conf, state)
    #threading.Thread(target=process, args=(conf, state)).start() #process on a different thread to not busy the server, but running on a new thread creates a RuntimeError
...
def process(conf, state):
    """
    Processes all data generated in the test.
    If there are other tests selected to run consecutively, it will start those after processing.
    """
...
def combine_files(file_names, output_name):
    """
    Combines multiple results files by appending columns
    """
    input = []
    max_lines = -1
